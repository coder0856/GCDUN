...
class GCIM(nn.Module):
    ...
    def forward(self, x, h, c, y,  Phi, PhiT):
        if h is None and c is None:
            ...
            c = self.conv2(self.res2(self.res1(self.conv1(self.cat(torch.cat([x,i*j],dim=1))))))
            h = o * c
...
class GCDUN(nn.Module):
    def __init__(self, sensing_rate, LayerNo):
        super(GCDUN, self).__init__()
    ...
    def forward(self, x):
        ...
        for i in range(self.LayerNo):
            if i == 0:
                c, h = self.fcs1[i](x, h, c,y, Phi, PhiT)
                his.append(c)
            else:
                x = his[i]
                c, h = self.fcs1[i](x, h, c,y, Phi, PhiT)
